...
ggplot(inflow_data, aes(x=my_date, y=HP3))

#adding two time series to the same plot
ggplot(inflow_data, aes(x=my_date)) +
  geom_line(aes(y=HP2, color="Reservoir 2")) +
  geom_line(aes(y=HP3, color="Reservoir 3")) +
  labs(y="Inflow", color="Legend")
```
...
```{r}
ts_inflow_data <- ts(inflow_data[, 2:4], 
                     start = c(1931, 1),
                     frequency = 12)
ts_inflow_data
```
...
HP1_acf <- acf(ts_inflow_data[, 1], lag.max=40,plot=TRUE)
...
HP1_acf <- Acf(ts_inflow_data[, 1], lag.max = 40)
HP1_pacf <- Pacf(ts_inflow_data[,1], lag.max=60, plot=TRUE)
