...
```{r}
#Importing data set
df_energy <- read_xlsx("../Data/Table_10.1_Renewable_Energy_Production_and_Consumption_by_Source.xlsx", skip=10)
```
...
```{r}
ts_energy <- ts(df_select, start=c(1973, 1), frequency=12)
```
...
```{r}
plot(ts_energy[,1], main="Total Biomass Production", ylab="Energy", col="blue")
abline(h=mean(ts_energy[,1]), col="red", lwd=2)
```
...
```{r}
energy_correlation <- cor(ts_energy)
...
```{r}
Acf(ts_energy[,1], lag.max=40, main="ACF - Biomass")
```
...
```{r}
Pacf(ts_energy[,1], lag.max=40, main="PACF - Biomass")
```
